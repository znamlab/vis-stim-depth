...
public class TubeMapping_NewZ_test2
{
    public IObservable<Tuple<int,float,Tuple<float,float,float>,Tuple<float,float,float,float,float>, Tuple<float,float>>> Process(IObservable<Tuple<Tuple<ElementIndex<Tuple<Tuple<double, double, double>,double>>, float, float, double>, double>> source)
        {
            return source.Select(value => 
            {
                ...
                float TubeSpacing = Convert.ToSingle(value.Item1.Item3);
                ...
                // Add MouseZ
                float Z;
                float VisibleLength = TubeSpacing * (SphereNumPerMoment);
                if (Z0 > VisibleLength)
                {
                    Z0 = VisibleLength; 
                }
