...
write2folder = 'Files_inputs\files_018t\';
...
if generate_translationXYZ
    ...
    % All angle values (theta &phi; rad) for calculating spherical coordinates
    tsl.azi.thetasss = deg2rad(linspace(tsl.azi.MIN,tsl.azi.MAX,tsl.x.n)); 
    tsl.lat.phisss = deg2rad(90 - linspace(tsl.lat.MAX,tsl.lat.MIN,tsl.y.n));
    tsl.r.values = linspace(tsl.r.MIN,tsl.r.MAX,tsl.z.n);
    
    
    % Create all combos of xyz in spherical coordinates
    tsl.xyz.values = [];
    for this_r = tsl.r.values
        for this_theta = tsl.azi.thetasss
            for this_phi = tsl.lat.phisss         
                this_x = this_r * cos(this_theta) * sin(this_phi);
                this_y = this_r * sin(this_theta) * sin(this_phi);
                this_z = this_r * cos(this_phi);
                tsl.xyz.values = [tsl.xyz.values;this_x,this_y,this_z,round(rad2deg(this_theta)),round(rad2deg(this_phi)), this_r];
            end
        end
    end
    ...
    if ifSave
        ...
        writetable(tsl.xyz.T(:,3),[root,tsl.y.filename,'.csv'],'WriteVariableNames',0); %Bonsai Y takes Matlab Z values
        writetable(tsl.xyz.T(:,2),[root,tsl.z.filename,'.csv'],'WriteVariableNames',0); %Bonsai Z takes Matlab Y values
    end
    
    disp(['MIN X = ',num2str(min(tsl.xyz.values(:,1)))]);
    disp(['MAX X = ',num2str(max(tsl.xyz.values(:,1)))]);
    disp(['MIN Y = ',num2str(min(tsl.xyz.values(:,3)))]);
    disp(['MAX Y = ',num2str(max(tsl.xyz.values(:,3)))]);
